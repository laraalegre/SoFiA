...
///                                                                      ///
/// SoFiA 1.3.3 (SoFiA.cpp) - Source Finding Application                 ///
...
void SoFiA::pipelineProcessReadStd()
{
	...
	outputStd.remove(QRegExp("\x1B\[[0-?]*[ -/]*[@-~]"));  // Get rid of ANSI escape sequences
...
void SoFiA::createInterface()
{
	...
	tabMergingForm1Middle = new QFormLayout();
	tabMergingForm1Right = new QFormLayout();
	tabMergingWidget1Left = new QWidget(tabMergingGroupBox1);
	tabMergingWidget1Middle = new QWidget(tabMergingGroupBox1);
	...
	tabMergingFieldMinSizeX = new QSpinBox(tabMergingWidget1Middle);
	...
	tabMergingFieldMinSizeX->setMaximum(99);
	connect(tabMergingFieldMinSizeX, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	tabMergingFieldMinSizeY = new QSpinBox(tabMergingWidget1Middle);
	...
	tabMergingFieldMinSizeY->setMaximum(99);
	connect(tabMergingFieldMinSizeY, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	tabMergingFieldMinSizeZ = new QSpinBox(tabMergingWidget1Middle);
	...
	tabMergingFieldMinSizeZ->setMaximum(99);
	connect(tabMergingFieldMinSizeZ, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	tabMergingFieldMaxSizeX = new QSpinBox(tabMergingWidget1Right);
	tabMergingFieldMaxSizeX->setObjectName("merge.maxSizeX");
	tabMergingFieldMaxSizeX->setMaximumWidth(100);
	tabMergingFieldMaxSizeX->setMinimum(-1);
	tabMergingFieldMaxSizeX->setMaximum(999);
	connect(tabMergingFieldMaxSizeX, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	tabMergingFieldMaxSizeY = new QSpinBox(tabMergingWidget1Right);
	tabMergingFieldMaxSizeY->setObjectName("merge.maxSizeY");
	tabMergingFieldMaxSizeY->setMaximumWidth(100);
	tabMergingFieldMaxSizeY->setMinimum(-1);
	tabMergingFieldMaxSizeY->setMaximum(999);
	connect(tabMergingFieldMaxSizeY, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	tabMergingFieldMaxSizeZ = new QSpinBox(tabMergingWidget1Right);
	tabMergingFieldMaxSizeZ->setObjectName("merge.maxSizeZ");
	tabMergingFieldMaxSizeZ->setMaximumWidth(100);
	tabMergingFieldMaxSizeZ->setMinimum(-1);
	tabMergingFieldMaxSizeZ->setMaximum(999);
	connect(tabMergingFieldMaxSizeZ, SIGNAL(valueChanged(int)), this, SLOT(parameterChanged()));
	...
	tabMergingForm1Middle->addRow(tr("Min. size X:"), tabMergingFieldMinSizeX);
	tabMergingForm1Middle->addRow(tr("Min. size Y:"), tabMergingFieldMinSizeY);
	tabMergingForm1Middle->addRow(tr("Min. size Z:"), tabMergingFieldMinSizeZ);
	tabMergingForm1Middle->setContentsMargins(0, 0, 0, 0);
	tabMergingForm1Right->addRow(tr("Max. size X:"), tabMergingFieldMaxSizeX);
	tabMergingForm1Right->addRow(tr("Max. size Y:"), tabMergingFieldMaxSizeY);
	tabMergingForm1Right->addRow(tr("Max. size Z:"), tabMergingFieldMaxSizeZ);
	tabMergingForm1Right->setContentsMargins(0, 0, 0, 0);
	tabMergingWidget1Left->setLayout(tabMergingForm1Left);
	tabMergingWidget1Middle->setLayout(tabMergingForm1Middle);
	tabMergingWidget1Right->setLayout(tabMergingForm1Right);
	
	tabMergingForm1Layout->addWidget(tabMergingWidget1Left);
	tabMergingForm1Layout->addWidget(tabMergingWidget1Middle);
	...
	tabParametrisationFieldSNRCut = new QLineEdit(tabParametrisationGroupBox2);
	tabParametrisationFieldSNRCut->setObjectName("reliability.fMin");
	tabParametrisationFieldSNRCut->setMaximumWidth(100);
	tabParametrisationFieldSNRCut->setMaxLength(10);
	tabParametrisationFieldSNRCut->setEnabled(true);
	connect(tabParametrisationFieldSNRCut, SIGNAL(textChanged(const QString &)), this, SLOT(parameterChanged()));
	...
	tabParametrisationForm2->addRow(tr("&sum;&thinsp;<var>S</var><sub><var>i</var></sub>&thinsp;/&thinsp;(&sigma;&thinsp;<var>N</var><sup>1/2</sup>) cut:"), tabParametrisationFieldSNRCut);
...
void SoFiA::createWhatsThis()
{
	...
	tabMergingFieldMaxSizeX->setWhatsThis(tr("<h3>merge.maxSizeX</h3><p>Maximum allowed extent of genuine sources in first dimension after merging. Sources above this size will be discarded. Set to -1 to disable maximum size filtering.</p>"));
	tabMergingFieldMaxSizeY->setWhatsThis(tr("<h3>merge.maxSizeY</h3><p>Maximum allowed extent of genuine sources in second dimension after merging. Sources above this size will be discarded. Set to -1 to disable maximum size filtering.</p>"));
	tabMergingFieldMaxSizeZ->setWhatsThis(tr("<h3>merge.maxSizeZ</h3><p>Maximum allowed extent of genuine sources in third dimension after merging. Sources above this size will be discarded. Set to -1 to disable maximum size filtering.</p>"));
	...
	tabParametrisationFieldSNRCut->setWhatsThis(tr("<h3>reliability.fMin</h3><p>Minimum value of the sum of the pixel values, &sum;<var>S</var><sub><var>i</var></sub>, divided by the global RMS noise level, &sigma;, and the square root of the number of pixels, <var>N</var>, for a source to be considered reliable. This parameter is a proxy for integrated signal-to-noise ratio, SNR<sub>int</sub> = &sum; <var>S</var><sub><var>i</var></sub> / [&sigma; <var>N</var><sup>1/2</sup> &Omega;<sup>1/2</sup>], where &Omega; is the solid angle of the beam, but has not yet been divided by the square root of the beam solid angle in pixels.</p>"));
