...
void SoFiA::createWhatsThis()
{
	...
	tabParametrisationFieldSNRCut->setWhatsThis(tr("<h3>reliability.fMin</h3><p>Minimum value of the sum of the pixel values, &sum;<var>S</var><sub><var>i</var></sub>, divided by the square root of the number of pixels, <var>N</var>, for a source to be considered reliable. This parameter is a proxy for integrated signal-to-noise ratio, SNR<sub>int</sub> = &sum; <var>S</var><sub><var>i</var></sub> / [&sigma; <var>N</var><sup>1/2</sup> &Omega;<sup>1/2</sup>], where &sigma; is the RMS noise level and &Omega; is the solid angle of the beam in pixels, but has not yet been divided by the square root of the beam solid angle in pixels.</p>"));
