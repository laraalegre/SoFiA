...
def sigma_scale(cube, scaleX=False, scaleY=False, scaleZ=True, edgeX=0, edgeY=0, edgeZ=0, statistic="mad"):
	...
	if statistic == 'mad': print ('Apply Median Absolute Deviation (MAD) statistic')
	if statistic == 'std': print ('Apply Standard Deviation (STD) statistic')
	if statistic == 'negative': print ('Apply Negatice statistic')
	sys.stdout.flush()
	...
	if scaleZ == True:
		...
		for i in range(len(z_rms)):
			if (not np.isnan(cube[i, y1:y2, x1:x2])).sum():
	...
	if scaleY == True:
		...
		for i in range(len(y_rms)):
			if (not np.isnan(cube[z1:z2, i, x1:x2])).sum():
	...
	if scaleX == True:
		...
		for i in range(len(x_rms)):
			if (not np.isnan(cube[z1:z2, y1:y2, i])).sum():
