...
def sigma_scale(cube, scaleX=False, scaleY=False, scaleZ=True, edgeX=0, edgeY=0, edgeZ=0, statistic="mad", localNoise=False, windowSizeSpatial=20, windowSizeSpectral=20, gridSizeSpatial=4, gridSizeSpectral=4):
	verbose = 0
	...
	if localNoise:
		...
		gridPointsZ = np.arange(gridSizeSpectral / 2, dimensions[0], gridSizeSpectral)
		gridPointsY = np.arange(gridSizeSpatial  / 2, dimensions[1], gridSizeSpatial)
		gridPointsX = np.arange(gridSizeSpatial  / 2, dimensions[2], gridSizeSpatial)
		gridPoints = []
		...
		for grid, window in list(zip(gridList, windowList)):
			if not np.all(np.isnan(cube[window[0]:window[1], window[2]:window[3], window[4]:window[5]])):
				rms_cube[grid[0]:grid[1], grid[2]:grid[3], grid[4]:grid[5]] = GetRMS(cube[window[0]:window[1], window[2]:window[3], window[4]:window[5]], rmsMode=statistic, zoomx=1, zoomy=1, zoomz=1, verbose=verbose, fluxRange="negative")
