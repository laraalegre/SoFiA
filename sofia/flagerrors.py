...
----------
  cube:            The input cube.
  flgthr:          Threshold for masking LOS's with anomalously high RMS. A LOS is masked if its RMS is flgthr*STD above the typical RMS of the cube,\
                   where STD is the width of the distribution of RMS values across all LOS's.
  dilside:         Side of the box structuring element used to propagate the flagging to neighbouring LOS. A side of 1 means no dilation.
"""

def flaglos(cube,flgthr,dilside):
	...
	# Mask all LOS's whose RMS is > flgthr*STD above rms0 (optionally extended to neighbouring LOS's using binary dilation with a box structuring element)
	los_rms_disp=np.nanstd(los_rms)
	los_rms=(los_rms<rms0+flgthr*los_rms_disp)
